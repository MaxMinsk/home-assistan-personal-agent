...
- `HAAG-048`: внедрен adaptive LLM routing слой (`off|shadow|enforced`) с детерминированным `LlmRoutingDecision` (model+reasoning), fallback policy `small -> default` при retryable provider/model ошибках, расширенной runtime/`/status` диагностикой и unit-тестами decision matrix + fallback + planner interaction.
- `HAAG-052`: `AgentRuntime` декомпозирован на orchestration facade + специализированные компоненты (`Preflight`, `ExecutionResolver`, `MafFactory`, `Runner`, `ToolCatalog`, `CompactionPipelineFactory`, `FallbackExecutor`, diagnostics/result/toolset resolvers), добавлены отдельные unit-тесты и документ `agent_runtime_architecture.md`.
- `HAAG-055`: upgraded persisted summary pipeline: delta-merge prompt contract + topic-shift/threshold/manual refresh reasons + summary quality/freshness diagnostics в `/status` + quality harness unit-tests.
- `HAAG-056`: переработан adaptive routing: intent classification (`simple/complex/tool/deep`), `simple_packed` context profile, deterministic `enforced` small-path для простых запросов и расширенная routing-диагностика в логах/`/status`.
...
### HAAG-054: Adaptive persona per user (эволюция личности ассистента)
