...
- Следующий приоритет по памяти:
  1. `HAAG-044` (retrieval mode switch),
  2. `HAAG-045` (memory hygiene/source attribution),
  3. `HAAG-042` (capsule artifacts/attachments),
  4. `HAAG-046` (production-grade vector backend/embeddings).
...
### HAAG-044: Retrieval mode switch для memory providers (BeforeAIInvoke vs OnDemandFunctionCalling)

Цель: сделать retrieval памяти управляемым по стоимости и latency, ближе к MAF `TextSearchProvider/ChatHistoryMemoryProvider`.

Что переделать:

- Добавить режимы retrieval: `before_invoke` и `on_demand_tool`.
- Для `on_demand_tool` экспонировать memory search как tool и не подмешивать retrieval в каждый run автоматически.
- Добавить конфиг-переключатель в add-on options и прозрачный статус в `/status`.

Acceptance criteria:

- Режим задается конфигом без перекомпиляции.
- В `before_invoke` память подмешивается автоматически; в `on_demand_tool` retrieval происходит только по tool call.
- В логах видно какой режим выбран и был ли выполнен retrieval.
- В коде/доке есть явные ссылки на `TextSearchProviderOptions.SearchBehavior` и `ChatHistoryMemoryProviderOptions.SearchBehavior`.
