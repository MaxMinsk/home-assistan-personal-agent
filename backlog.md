...
- `HAAG-051`: реализован Telegram live reasoning preview (ephemeral): streaming reasoning deltas при долгом ответе, временное preview-сообщение с авто-редактированием/удалением и add-on настройки `reasoning_preview_enabled` + `reasoning_preview_delay_seconds`.

## Ready

### HAAG-048 (High Priority): Adaptive LLM execution routing (model + reasoning) для снижения токенов и стоимости

Цель: объединить выбор модели и thinking/reasoning в один детерминированный routing-слой, чтобы снизить latency/стоимость без просадки качества и стабильности tool-loop.

Контекст по MAF (что берем за паттерн):

- Per-request middleware/policy слой для принятия решения до вызова провайдера (`Agent_Step11_Middleware`, `AgentsWithFoundry/Agent_Step12_Middleware`).
- Детерминированный routing как отдельный decision-узел (`Workflows/ConditionalEdges` samples).
- Cost-aware подход с разными моделями по подзадачам (пример: отдельный cheaper summarizer в `Agent_Step18_CompactionPipeline`).

Что переделать:

- Объединить логику в единый `LlmRoutingDecision` перед вызовом LLM:
  - `model`: `small_context_fast | default_model`;
  - `reasoning`: `disabled | provider-default | deep`.
- Реализовать router в 2 фазах:
  1. `shadow` — decision только логируется;
  2. `enforced` — decision реально применяется.
- Базовый router сделать детерминированным (без дополнительного LLM вызова):
  - короткие бытовые/factoid/операционные запросы + малый контекст -> `moonshot-v1-8k` + `reasoning=disabled`;
  - сложные аналитические/планирование/большой контекст -> `default_model` + `provider-default/deep`;
  - явный intent (`/think`, "пошагово") -> `deep`.
- Встроить provider/tool safety guardrails:
  - сохранять текущую Moonshot safety-политику `reasoning_content`;
  - auto-fallback на `default_model`, если small model не подходит по ограничениям контекста;
  - retry один раз на `default_model` при provider/model error для small-model пути.
- Добавить add-on конфиг:
  - `llm_router_mode` (`off|shadow|enforced`);
  - `llm_router_small_model` (default: `moonshot-v1-8k`);
  - `llm_router_max_input_chars_for_small`;
  - `llm_router_max_history_messages_for_small`;
  - `llm_router_deep_keywords` (опционально).
- Добавить диагностику и cost observability:
  - `llm_router_decision`, `llm_router_reason`, `llm_router_fallback`;
  - usage/tokens/latency по bucket'ам (`small+disabled`, `default+provider-default`, `default+deep`);
  - override/fallback counters в `/status` и runtime-логах.

Acceptance criteria:

- В `shadow` режиме виден полный decision per run без изменения поведения.
- В `enforced` режиме простые запросы стабильно уходят в `moonshot-v1-8k`, сложные остаются на default model.
- Нет регрессий по tool-calls/confirmation/памяти и reasoning safety fallback.
- Есть тесты на routing decision matrix + fallback path (`small -> default`) + thinking policy interaction.
- На representative сценариях зафиксирована measurable экономия стоимости и/или latency.
- В `agent_reasoning_flow.md` добавлен блок про unified routing layer и decision pipeline.

### HAAG-049: Telegram long-response chunking без обрезания ответа модели

Цель: перестать обрезать длинные ответы модели в Telegram и доставлять полный ответ пользователю через несколько последовательных сообщений.

Что переделать:

- Реализовать централизованный chunker для Telegram-текста с учетом лимита Telegram на длину одного сообщения.
- Делить ответ по безопасным границам (абзац/строка/пробел), чтобы не ломать markdown и читаемость.
- Отправлять chunks последовательно в правильном порядке, с корректной обработкой ошибок/retry.
- Сохранить текущий UX typing indicator на время отправки длинного ответа.
- Добавить диагностику: сколько chunks отправлено и общая длина исходного ответа.

Acceptance criteria:

- Ответы длиннее лимита Telegram приходят полностью (все части), без потери хвоста.
- Порядок частей всегда стабильный.
- Короткие ответы продолжают отправляться одним сообщением без изменений.
- Есть unit-тесты на chunking (обычный текст, многострочный текст, длинные слова/границы).
