...
- **Your agents stop being silos.** With a per-agent switch, "Видеть находки других агентов" (off by default), a run can see what your *other* agents recently found and are focused on, so it can point out real connections — strictly grounded, it will never invent a link. And when several agents wake up in the same window, their results now arrive as **one consolidated digest** (a per-agent overview plus a short "Связи" block) instead of a stack of separate messages; each agent's questions still come as their own message so replies and approvals keep working. (HPA-039.)
- **Slash-command palette in the panel chat.** Type `/` in the chat and a menu of commands appears — `/help`, `/status`, `/summary`, `/reset`, and `/fast` · `/deep`. They run locally (nothing is sent to the model) and show their result as a small info bubble. (HPA-048.)
- **The agent status now reads correctly after you reply.** Once your answer is queued for an agent's next run, its status shows "ответ в очереди" instead of still saying "ждёт ответа". (HPA-050.)
- **Optional: keep thinking on while using tools (experimental).** A new add-on option, `llm_replay_reasoning_to_wire` (off by default), makes the model keep reasoning *between* tool calls rather than only before the first one, by echoing its own reasoning back on each tool step. It is opt-in because some providers may reject the echoed reasoning; turn it on and check that answers still work on your key, and turn it back off if they don't. Diagnostics in the add-on log show whether it is actually taking effect. (HPA-041, HPA-051.)
- Internal: retired an inert reasoning-replay middleware whose output never reached the wire and whose warnings were misleading; the add-on log now states the real reason thinking is toggled on a tool step. New per-agent columns (`allow_propose_actions`, `allow_cross_agent_context`) are migrated in automatically on existing installs. Delivery of background results was decoupled from the run so the scheduler can consolidate a digest. New endpoints `GET /api/agents/{id}/actions` and `POST /api/agents/{id}/actions/{actionId}/approve|reject`. Sprint HPA-S6: HPA-035, HPA-039, HPA-041, HPA-048, HPA-050, HPA-051.
