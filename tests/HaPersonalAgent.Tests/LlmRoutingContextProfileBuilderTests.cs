...
/// Как: проверяет подрезку history/summary, сохранение retrieved memory (для personal-fact вопросов) и профиль выполнения в simple-path.
/// </summary>
public class LlmRoutingContextProfileBuilderTests
{
    [Fact]
    public void BuildSimplePacked_trims_history_and_summary_and_disables_tools_but_keeps_memory()
    {
        ...
        // Long-term memory must survive the cost route — dropping it blinded the assistant on
        // short personal-fact questions and caused confabulation.
        Assert.Equal("retrieved-memory-context", profile.Context.RetrievedMemoryContext);
        Assert.Equal(3, profile.Context.RetrievedMemoryCount);
        Assert.Equal("retrieved-memory-context".Length, profile.RetrievedMemoryChars);
