...
- For architecture and behavior decisions, prioritize implementation patterns from official Microsoft Agent Framework samples/ADR docs first, and only deviate with an explicit local reason.
- In implementation comments/docs, include explicit source links to the exact MAF sample/ADR files used as reference (prefer repository paths + URL when useful).
